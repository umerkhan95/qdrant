...
use crate::universal_io::{self, ElementsRange, OpenOptions, UniversalRead};
    ...
    pub(super) fn get_from_cache<O>(
        ...
        match self.cache.get_value_or_guard(&key, None) {
            ...
            GuardResult::Guard(guard) => {
                ...
                let elem_range = ElementsRange {
                    start: key.offset.bytes() as u64,
                    length: BLOCK_SIZE as u64,
                };

                let cow = file.read::<false>(elem_range)?;
